...
CORRECT_PIN = 'fruitsJuice'
MAX_ATTEMPTS = 5

# i は 0, 1, 2 と変化し、ループは3回実行されます
for i in range(MAX_ATTEMPTS):
    
    # {i + 1} で「1回目」「2回目」と表示できます
    pin = input(f'PINを入力（{i + 1}回目）: ') 
...
# 【重要】ループの直後にあるこの else ブロックにご注目ください！
else:
    # このブロックは、forループが break されずに「全て実行された」場合にのみ動きます。
    # つまり、「3回全て間違えた」場合のみ実行されます。
    print('試行回数が上限に達しました。終了します。')


num = input(f'numを入力（{i+3*(2 + 1)}回目）: ') 
print(num)